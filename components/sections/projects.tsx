      ...
      <div className="p-5 sm:p-6 md:p-8">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <span
              className="font-mono text-[10px] tracking-[0.1em] uppercase sm:text-[11px]"
              style={{ color: categoryColors[project.category] }}
            >
              {project.category}
            </span>
            <h3 className="mt-1.5 text-base font-light tracking-tight text-white sm:text-lg md:text-xl">
              {project.title}
            </h3>
            <p className="mt-0.5 text-xs text-[#666] sm:text-sm">{project.subtitle}</p>
          </div>
          <span className="shrink-0 font-mono text-[10px] text-[#444] sm:text-xs">
            {project.year}
          </span>
        </div>

        {/* Description */}
        <p className="mt-4 text-xs leading-relaxed text-[#a3a3a3] sm:mt-5 sm:text-sm sm:leading-[1.65]">
          {project.description}
        </p>
        ...
        <div className="mt-4 flex flex-wrap gap-1.5 sm:mt-5 sm:gap-2">
          {project.tags.map((tag) => (
            <span
              key={tag}
              className="bg-[#111] px-2 py-0.5 font-mono text-[9px] tracking-[0.06em] uppercase text-[#666] sm:px-2.5 sm:py-1 sm:text-[10px]"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="mt-4 flex items-center gap-4 sm:mt-5">
          {project.url && (
            <a
              ...
              className="flex min-h-[32px] items-center gap-1.5 text-[11px] text-[#666] transition-colors hover:text-white sm:text-xs"
            >
          ...
          <div className="flex gap-1 overflow-x-auto pb-1">
            {categories.map((cat) => (
              <button
                key={cat}
                data-filter={cat}
                className={`shrink-0 px-3 py-1.5 font-mono text-[10px] tracking-[0.08em] uppercase transition-all duration-300 min-h-[36px] sm:px-4 sm:py-2 sm:text-[11px] ${
                  cat === "all"
                    ? "bg-white text-black"
